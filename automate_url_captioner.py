...
# Load the pretrained processor and BLIP model
...
with open("captions.txt", "w", encoding="utf-8") as caption_file:
    for idx, img_element in enumerate(img_elements, start=1):
        # Extract image URL from attributes
        ...
        try:
            ...
            # Skip very small icons/thumbnails
            ...
            print(f"[{idx}] Caption saved for {img_url}")

        except OSError:
            # Skip images PIL cannot handle
            continue
        except Exception as e:
            print(f"[{idx}] Error with {img_url}: {e}")
            continue
